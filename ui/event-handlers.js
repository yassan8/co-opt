 ...
 */
export function setupTransformationControls() {
  const transformSurfaceSelect = document.getElementById('transform-surface-select');
  const showLocalCoordsBtn = document.getElementById('show-local-coords-btn');
  const cancelTransformBtn = document.getElementById('cancel-transform-btn');
  const saveLocalCoordsBtn = document.getElementById('save-local-coords-btn');
  const errorBar = document.getElementById('transform-error-bar');
  const errorText = document.getElementById('transform-error-text');
  const progressWrapper = document.getElementById('transform-progress-wrapper');
  const progressText = document.getElementById('transform-progress-text');
  const progressBar = document.getElementById('transform-progressbar');
  ...
  if (showLocalCoordsBtn) {
    showLocalCoordsBtn.addEventListener('click', async function() {
      ...
      try {
        ...
        window._transformCalculationCancelled = false;
        ...
        // Store results
        window._cachedLocalCoords = result;
        window._showLocalCoords = true;
        
        // Redraw table
        if (window.tableOpticalSystem) {
          window.tableOpticalSystem.redraw();
        }
  ...
  if (cancelTransformBtn) {
    cancelTransformBtn.addEventListener('click', function() {
      window._transformCalculationCancelled = true;
      if (cancelTransformBtn) cancelTransformBtn.style.display = 'none';
      hideProgress();
  ...
  if (saveLocalCoordsBtn) {
    saveLocalCoordsBtn.addEventListener('click', function() {
      try {
        if (!window._cachedLocalCoords) {
          showError('No coordinate data to save. Please calculate first.');
          return;
        }
        
        const data = window._cachedLocalCoords;
        const json = JSON.stringify(data, null, 2);
        const blob = new Blob([json], { type: 'application/json' });
        const url = URL.createObjectURL(blob);
 ...
 */
export function updateTransformSurfaceSelect() {
  const transformSurfaceSelect = document.getElementById('transform-surface-select');
