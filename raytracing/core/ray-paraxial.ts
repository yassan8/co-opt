...
export function calculatePupilsByNewSpec(opticalSystemRows, wavelength = 0.5875618) {
  try {
    ...
    const exitPupil = calculateStrictExitPupil(opticalSystemRows, stopIndex, stopRadius, wavelength);
    const entrancePupil = calculateStrictEntrancePupil(opticalSystemRows, stopIndex, stopRadius, wavelength);

    return {
      exitPupil: exitPupil,
      entrancePupil: entrancePupil,
      isValid: !!exitPupil?.isValid && !!entrancePupil?.isValid,
      stopIndex: stopIndex,
...
export function calculateEntrancePupilDiameter(opticalSystemRows, wavelength = 0.5875618) {
  ...
  try {
    ...
    const entrancePupilDetails = calculateStrictEntrancePupil(opticalSystemRows, stopIndex, stopRadius, wavelength);
    if (entrancePupilDetails && Number.isFinite(entrancePupilDetails.diameter)) {
...
export function calculateExitPupilDiameter(opticalSystemRows, wavelength = 0.5875618) {
  ...
  try {
    ...
    const stopDiameter = stopRadius * 2;
    const exitPupilDetails = calculateStrictExitPupil(opticalSystemRows, stopIndex, stopRadius, wavelength);
    if (exitPupilDetails && Number.isFinite(exitPupilDetails.diameter)) {
      return {
        diameter: exitPupilDetails.diameter,
        position: exitPupilDetails.position,
        magnification: exitPupilDetails.magnification,
...
export function getRefractiveIndex(surface, wavelength = 0.5875618) {
  ...
  if (effectiveMaterial && effectiveMaterial !== '' && effectiveMaterial !== 'Air' && effectiveMaterial !== 'AIR' && effectiveMaterial !== 'empty') {
    try {
      ...
      if (glassData) {
        // 指定波長での屈折率を計算
        if (String(glassData.manufacturer || '').toUpperCase() === 'HIKARI' && glassData.dispersion) {
          return calculateRefractiveIndexHikari(glassData.dispersion, wavelength);
        } else if (String(glassData.manufacturer || '').toUpperCase() === 'HIKARI' && glassData.nd && glassData.vd) {
...
function findFirstLastPhysicalSurfaceIndex(opticalSystemRows) {
  ...
  for (let i = 1; i < opticalSystemRows.length - 1; i++) {
    const surface = opticalSystemRows[i];
    if (!surface || isCoordTransSurface(surface)) continue;
    if (first < 0) first = i;
    last = i;
  }
  return { first, last };
...
function resolveNextMediumIndex(surface, prevN, wavelength, meridian = 'average') {
  ...
  if (!Number.isFinite(nextN) || Math.abs(nextN) < 1e-12) nextN = 1.0;
  ...
  let thickness = getSafeThickness(surface);
  if (!Number.isFinite(thickness)) thickness = 0;

  return { nextN, phi, thickness };
}

function computeMediumBeforeSurfaceIndex(opticalSystemRows, surfaceIndex, wavelength, meridian = 'average') {
  let prevN = 1.0;
  if (!Array.isArray(opticalSystemRows) || surfaceIndex <= 1) return prevN;

  for (let j = 1; j < surfaceIndex; j++) {
    const surface = opticalSystemRows[j];
    if (!surface || isCoordTransSurface(surface)) continue;
    const { nextN } = resolveNextMediumIndex(surface, prevN, wavelength, meridian);
    prevN = nextN;
  }
  return prevN;
}
...
function buildParaxialTransferMatrix(opticalSystemRows, startSurfaceIndex, endSurfaceIndex, wavelength = 0.5875618, meridian = 'average') {
  // Matrix maps [h; alpha] at start surface plane to [h; alpha] at end surface plane.
  const identity = [[1, 0], [0, 1]];
  if (!Array.isArray(opticalSystemRows)) {
    return { matrix: identity, nIn: 1.0, nOut: 1.0 };
  }

  const start = Number(startSurfaceIndex);
  const end = Number(endSurfaceIndex);
  if (!Number.isInteger(start) || !Number.isInteger(end) || end <= start) {
    const nSame = computeMediumBeforeSurfaceIndex(opticalSystemRows, Math.max(1, Number.isInteger(start) ? start : 1), wavelength, meridian);
    return { matrix: identity, nIn: nSame, nOut: nSame };
  }

  let m = identity;
  let prevN = computeMediumBeforeSurfaceIndex(opticalSystemRows, start, wavelength, meridian);
  const nIn = prevN;
  ...
  for (let j = start; j < end; j++) {
    ...
    const { nextN, phi, thickness } = resolveNextMediumIndex(surface, prevN, wavelength, meridian);
    const tOverN = Math.abs(nextN) > 1e-12 ? (thickness / nextN) : 0;
    const step = [
      [1 - tOverN * phi, -tOverN],
      [phi, 1],
    ];
    m = multiply2x2(step, m);
    prevN = nextN;
  }

  return { matrix: m, nIn, nOut: prevN };
}

function calculateStrictEntrancePupil(opticalSystemRows, stopIndex, stopRadius, wavelength = 0.5875618, meridian = 'average') {
  const stopDiameter = Math.abs(Number(stopRadius) || 0) * 2;
  if (!(stopDiameter > 0)) {
    return { position: 0, diameter: null, magnification: null, isValid: false };
  }

  const { first } = findFirstLastPhysicalSurfaceIndex(opticalSystemRows);
  if (first < 0 || stopIndex <= first) {
    return { position: 0, diameter: stopDiameter, magnification: 1, isValid: true };
  }

  const { matrix, nIn } = buildParaxialTransferMatrix(opticalSystemRows, first, stopIndex, wavelength, meridian);
  const A = Number(matrix?.[0]?.[0]);
  const B = Number(matrix?.[0]?.[1]);
  if (!Number.isFinite(A) || Math.abs(A) < 1e-12 || !Number.isFinite(B)) {
    return { position: 0, diameter: stopDiameter, magnification: 1, isValid: false };
  }

  // Conjugate condition with translation on object side: B_total = A*(-L/n0) + B = 0
  const n0 = (Number.isFinite(nIn) && Math.abs(nIn) > 1e-12) ? nIn : 1.0;
  const lFromFirstSurface = (B * n0) / A;
  const magnification = 1 / A;
  const entranceDiameter = Math.abs(magnification) * stopDiameter;

  // Positive means image-side; negative means object-side from first surface vertex.
  const position = -lFromFirstSurface;
  return {
    position,
    diameter: Number.isFinite(entranceDiameter) ? entranceDiameter : null,
    magnification,
    isValid: Number.isFinite(position) && Number.isFinite(entranceDiameter),
  };
}

function calculateStrictExitPupil(opticalSystemRows, stopIndex, stopRadius, wavelength = 0.5875618, meridian = 'average') {
  const stopDiameter = Math.abs(Number(stopRadius) || 0) * 2;
  if (!(stopDiameter > 0)) {
    return { position: 0, diameter: null, magnification: null, isValid: false };
  }

  const { last } = findFirstLastPhysicalSurfaceIndex(opticalSystemRows);
  if (last < 0 || stopIndex >= last) {
    return { position: 0, diameter: stopDiameter, magnification: 1, isValid: true };
  }
  ...
  // Positive means image-side from last physical surface vertex.
  const position = lFromLastSurface;
  return {
    position,
    diameter: Number.isFinite(exitDiameter) ? exitDiameter : null,
    magnification,
    isValid: Number.isFinite(position) && Number.isFinite(exitDiameter),
  };
}
...
export function calculateParaxialData(opticalSystemRows, wavelength = 0.5875618) {
  try {
    ...
    if (stopIndex !== -1) {
      const stopSurface = opticalSystemRows[stopIndex];
      const stopRadius = Number(stopSurface?.semidia || stopSurface?.["Semi Diameter"] || 0);
      exitPupilDetails = calculateStrictExitPupil(opticalSystemRows, stopIndex, stopRadius, wavelength);
      if (!exitPupilDetails || !Number.isFinite(exitPupilDetails.diameter)) {
