...
function normalizePatentOcrText(input) {
  let text = String(input ?? '')
    ...
    .replace(/[−–—]/g, '-')
    .replace(/[，]/g, ',')
    // Match something like "x 10”" where the exponent is missing or malformed
...
function parsePatentAsphereTerms(line) {
  ...
  // Adjusted to handle "AL", "Ad", and potential "--" or "-" properly, and allow order before prefix
  // Pattern: (Optional Prefix) (Order) (Optional Prefix) (Optional separators) (Value)
  const orderPattern = new RegExp(`(?:\\b(A|Ad|AL|r|d|v|N|M|E|F|G|H|I|J|K|L|P|Q|S|T|U|V|W|Z))?\\s*(\\d{1,2})\\s*(?:(A|Ad|AL|r|d|v|N|M|E|F|G|H|I|J|K|L|P|Q|S|T|U|V|W|Z))?\\s*[=:-]*\\s*(-?\\s*-?\\s*${PATENT_NUMBER_SOURCE})`, 'gi');
  
  for (const match of text.matchAll(orderPattern)) {
    const prefix1 = match[1];
    const order = Number(match[2]);
    const prefix2 = match[3];
    const rawValue = match[4].replace(/\s+/g, '').replace('--', '-');
    const value = Number(rawValue);
    
    if (!Number.isFinite(order) || !Number.isFinite(value)) continue;
    if (!prefix1 && !prefix2) continue; // Must have some prefix like A, Ad, etc.
