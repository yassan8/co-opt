...
function buildGlassPoints(): GlassPoint[] {
  ...
  for (const db of dbs) {
    ...
    for (const g of db) {
      ...
      points.push({
        ...
        nd,
        vd,
        price: isFiniteNumber((g as any).price) ? Number((g as any).price) : null,
      });
    }
  }
 ...
 *
 * @param onRegionSelected - Callback when user selects a region
 * @param onGlassSelected - Optional callback when user double-clicks a glass
 * @returns The popup window or null if blocked
 */
export function openGlassMapWindow(
  onRegionSelected: (region: RegionSelection) => void,
  onGlassSelected?: (glass: GlassSelection) => boolean | void
): Window | null {
  ...
  (w as any).__COOPT_GLASS_POINTS__ = points;
  (w as any).__COOPT_ON_REGION_SELECTED__ = onRegionSelected;
  (w as any).__COOPT_ON_GLASS_SELECTED__ = typeof onGlassSelected === 'function' ? onGlassSelected : null;
